...
subroutine readmeshdata(p,e,t,np,ne,nt,extension)
    ...
    filename = 'points' // trim(extension) // '.dat'
    ...
    filename = 'edges' // trim(extension) // '.dat'
    open(unit=1,file=filename,status='old')
    do i = 1,ne
        read(unit=1,fmt=*) e(:,i)
    end do
    ...
    filename = 'triangles' // trim(extension) // '.dat'
    open(unit=1,file=filename,status='old')
    do i = 1,nt
        read(unit=1,fmt=*) t(:,i)
    end do
...
subroutine readmeshdim(np,ne,nt,nb,extension)
    ...
    filename = 'meshdim' // trim(extension) // '.dat'
    open(unit=1,file=filename,status='old')
    read(1,*) np, ne, nt, nb
    close(1)
...
subroutine readPceData(nord, ndim, npceout, npcein)
    ...
    open(unit=2,file='pcedata.dat',status='old')
...
subroutine readIndices(nDim,npceout,mIndex,sIndex)
    ...
    open(unit=2,file='sortIndex.dat',status='old')
    do i = 1,nDim
...
subroutine readprocess(nomga,omegas,multipliers)
    ...
    open(unit=1,file='omegas',status='old')
    read(1,*) omegas
    close(1)
    open(unit=1,file='multipliers',status='old')
    read(1,*) multipliers
    close(1)
...
open(unit=1,file="cijk",status='old')
...
subroutine getub(pid,nb,nbg,npceout,Ub_g,Ub)
   ...
   character(len=255) :: str1
   integer :: nb, nbg, i, j, k, l, pid, temp1, npceout
...
filename = 'nmeas' // trim(extension) // '.dat'
open(unit=1,file=filename,status='old')
read(1,*) nmeas
close(1)
...
filename = 'measnodes' // trim(extension) // '.dat'
open(unit=1,file=filename,status='old')
do i = 1,nmeas
   read(1,*) measnodes(i)
end do
